...
public class Lotto {
    ...
    public Looto(List<Lotto> numbers) {
...
위 코드는 두 가지 문제점이 발생할 수 있음.

첫 번째는 생성자의 파라미터로 주어진 LottoNumber 리스트의 각 요소가 외부에서 변경될 수 있는 가능성이 존재함. 이러한 문제가 발생하는 이유는 방어적 복사가 깊은 복사가 아니기 때문임. 가령, 외부에서 다음과 같은 코드를 작성할 수 있음.
